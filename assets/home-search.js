    ...
    list.innerHTML = slice.map(({ record, matchedFields }) => {
      ...
      const facts = [
        view.category.label === 'Judicial case' && (view.proceedingType || view.reviewStandard)
          ? `<span class="home-result__fact home-result__fact--judicial"><b>Case:</b> ${view.proceedingType ? esc(view.proceedingType) : ''}${view.proceedingType && view.reviewStandard ? ' · ' : ''}${view.reviewStandard ? `<b>Basis:</b> ${esc(view.reviewStandard)}` : ''}</span>` : '',
