...
def main(argv=None):
    ...
    for year, year_rows in groups.items():
        ...
        for row in year_rows:
            ...
            source_label = "Read" if row.get("case_page") else "View official PDF for"
            source_markup = f'<a href="{esc(href)}" aria-label="{source_label} {esc(title)}">{"Read case" if row.get("case_page") else "Official PDF"} <span aria-hidden="true">→</span></a>' if href else '<span class="judicial-source-missing">Full text unavailable</span>'
            ...
            actions_id = "judicial-actions-menu-" + "".join(ch if ch.isalnum() else "-" for ch in str(record_id))
            actions_button_id = "judicial-actions-button-" + "".join(ch if ch.isalnum() else "-" for ch in str(record_id))
            lines.extend([
                ...
                '<div class="judicial-case__heading">',
                ...
                '</div>',
                f'<aside class="judicial-case__rail" aria-label="Actions for {esc(title)}">{source_markup}<button type="button" class="judicial-details__toggle" aria-controls="{details_id}" aria-expanded="false" aria-label="Show details for {esc(title)}">Show details</button><div class="judicial-actions"><button type="button" class="judicial-actions__button" id="{actions_button_id}" aria-haspopup="menu" aria-controls="{actions_id}" aria-expanded="false" aria-label="More actions for {esc(title)}">More actions</button><div class="judicial-actions__menu" id="{actions_id}" role="menu" aria-labelledby="{actions_button_id}" hidden><button type="button" role="menuitem" data-judicial-action="save">Save to bookshelf</button><button type="button" role="menuitem" data-judicial-action="cite">Copy citation</button><button type="button" role="menuitem" data-judicial-action="link">Copy link</button></div></div></aside>',
