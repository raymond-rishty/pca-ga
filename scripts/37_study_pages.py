...
def pdf_text_artifact_section(r: dict) -> list[str]:
    ...
    return [
        "## PDF text artifact", "",
        f"Audit artifact: [`{artifact}`](../{artifact}).", "",
        "This PDF text is retained only for locator/audit review. Study pages do not copy "
        "PDF-derived text as document content; when a reliable mapping exists, full text is "
        "generated by slicing the corresponding `markdown/ga*.md` minutes range.", "",
        ...
        if r.get("external_url"):
            ...
            if r.get("full_text_sources"):
                ...
                body += [
                    ...
                    "located for it. The PDF text artifact is linked for audit only and is not copied "
                    "as page content.*", "",
                ]
