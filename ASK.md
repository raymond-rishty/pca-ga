...
1. BCO wording or interpretation: read the provision and relevant cross-references. For current wording, follow the official BCO contents at https://www.pcaac.org/book-of-church-order/ and identify the edition checked. For reader text and Westminster Standards, use https://raymond-rishty.github.io/pca-constitution-reader/llms.txt. If the reader's hash link returns only an app shell, fetch the corresponding content file listed there; BCO text is in content/bco.js. A wording-only question can stop with verified text. For interpretation or application, also check the Minutes evidence below.
...
Before citing a historical claim, verify the record's identity, the supporting passage, who said it, and the actual disposition. Distinguish judicial judgments from arguments/dissents, CCB non-binding advice, RPR exceptions and responses, proposals, adopted Assembly actions, and study-report reception or adoption. Do not treat an index summary or authority_weight as the source's authority. Separate what a source states from your inference.

Cite the underlying published .html record with its case/item identifier and verified Minutes page. Read the PAGE comment in raw Markdown if necessary: ga and printed_page give M<GA>GA p.<page>. URL fragments and pdf_page are locators; never infer printed pagination from them. Copy an existing anchor or use the record's base URL. If printed_page is missing, label the available locator and the pagination limit. For current BCO wording, cite the official text checked; reader links can accompany it. Use source PDFs when extraction, pagination, or wording needs verification.
