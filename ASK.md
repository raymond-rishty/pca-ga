...
Cite the underlying published .html record with its case/item identifier and verified Minutes page. Include a clickable link to every underlying source used for a material claim; do not link only to a manifest, catalogue, search result, or machine-readable asset. Read the PAGE comment in raw Markdown if necessary: ga and printed_page give M<GA>GA p.<page>. URL fragments and pdf_page are locators; never infer printed pagination from them. Copy an existing anchor or use the record's base URL. If printed_page is missing, label the available locator and the pagination limit. For every constitutional provision cited, include its reader-facing link (for example, https://raymond-rishty.github.io/pca-constitution-reader/#bco/38-4 or #wlc/Q.135). For current BCO wording, also cite the official HTML text checked. Read corpus Markdown first, and use a source PDF only when OCR, layout, wording, or pagination remains uncertain.
