...
2. Known provision: fetch its schema-v2 JSON directly from `api/provisions/{book}/{reference}.json`, for example https://raymond-rishty.github.io/pca-ga/api/provisions/bco/38-4.json. Use api/provisions/index.json to find routes. Existing api/bco/*.json paths are compatible BCO aliases with the same schema-v2 content; api/bco/index.json lists those aliases. Chapter and section objects are not rollups of every subsection. Follow relationship occurrences to their source records and exact available locators.
...
The generated provision catalogue and its projections are finding aids. Provision tags are not exhaustive, and a citation may occur in a party's argument or dissent. Relationship evidence basis describes how a link was found; relevance is unreviewed unless explicitly curated. Search relevant catalogues by topic as well as provision; recommendation and study coverage is incomplete. Follow later action, related cases, RPR responses, and an overture's referenced answer or amendment ratification when they could affect the conclusion. For historical wording, check the text then in force and date-specific predecessor numbers; renumbering data may be partial. Expand to volume outlines and full Minutes when catalogue evidence is insufficient. Use llms-full.txt only as an optional combined-catalogue fallback; it is not the full Minutes or every catalogue.
